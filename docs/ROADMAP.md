...
| Input and output | Mostly done: strings and their intrinsics (including `Split` and `Regexp`), `print` with print levels, `printf`/`fprintf`/`Sprintf` (widths, `%o %O %m %h`), `Sprint`, previous values `$1`..., indentation, `PrintFile`, `SetOutputFile`, `SetLogFile`, file objects (`Open`, `Gets`, `Puts`, ...), `Pipe`, `System`, `POpen`, binary strings (`ReadBinary`, `WriteBinary`), `load`/`iload`, `read`/`readi`. **Missing:** sockets, asynchronous I/O, `ReadObject`/`WriteObject`, `save`/`restore` ([#27](https://github.com/aygerix/calyx-math/issues/27)). |
| Environment and options | Done, pending validation against 2.29 ([#24](https://github.com/aygerix/calyx-math/issues/24)): `-b -e -h -n -s -S -V` and `name:=value` arguments, set/get intrinsics (`SetColumns`, `SetAssertions`, `SetVerbose`, `SetHistorySize`, `SetViMode`, `SetLibraryRoot`, ...), `ShowIdentifiers`, `ShowValues`, `ListSignatures`, `ListCategories`, `?Name` help, the history commands (`%p %P %s %S`, ranges, recall, `%e`, `%!`), and the environment variables under calyx's own names (`CALYX_PATH`, `CALYX_STARTUP_FILE`, ...; the README lists them). Not implemented: `CALYX_MEMORY_LIMIT` and `CALYX_HELP_DIR`. |
...
| Chapter | Status |
| --- | --- |
| Matrices | In progress ([#76](https://github.com/aygerix/calyx-math/issues/76)): matrices and vectors on FLINT's native matrix types over the entry ring, with matrix algebras, matrix spaces and R-spaces as parents; creation (text/270), elementary properties (text/271), access and modification of entries (text/272), printing with Magma's column alignment, elementary arithmetic (text/275), block matrices (text/273), changing rings (text/274), nullspaces and solutions of systems (text/276), predicates (text/277), determinants, ranks, minors, Pfaffians and the other properties of text/278 (`Rank` also over multivariate polynomial rings), characteristic and minimal polynomials, their factored forms, eigenvalues and eigenspaces (text/279), the canonical forms (text/280: echelon, Hermite, Smith, Hessenberg, rational, primary rational and Jordan forms, invariant factors and elementary divisors, `Saturation`, `IsSimilar`, `FrobeniusFormAlternating`; transformation matrices are valid but calyx's own), numerical linear algebra over the real and complex fields (text/282: RQ and QL decompositions, numerical inverse, rank, kernel, image, solutions and pseudoinverse, Hessenberg and Schur forms, eigenvalues and eigenvectors, bidiagonal form and singular value decomposition), and orders of invertible matrices (text/281: `Order`, `FactoredOrder`, `ProjectiveOrder` and their factored forms, `HasFiniteOrder`). **Missing:** `FrobeniusImage` (text/283). |
| Sparse matrices | Done, pending validation against 2.29 ([#77](https://github.com/aygerix/calyx-math/issues/77)): creation, access, entries and blocks, joins, conversion to and from dense matrices, changing rings, predicates, arithmetic, products with dense vectors and matrices, nullspaces, ranks, determinants, minimal and characteristic polynomials, eigenvalues and elementary divisors, and `ModularSolution` by structured Gaussian elimination. |
| Vector spaces | In progress ([#78](https://github.com/aygerix/calyx-math/issues/78)): vector spaces and R-spaces with an inner product matrix (`VectorSpace`, `KSpace` and `RSpace` with a form, `InnerProductMatrix`, `InnerProduct`, `Norm`, basis vectors `V.i`), from text/304. |
| Polar spaces | In progress ([#79](https://github.com/aygerix/calyx-math/issues/79)): inner products (text/316: dot products and their matrices, Gram matrices, orthogonal complements and radicals, degeneracy, singular radicals) and the standard forms (text/318). **Known differences:** the minus-type quadratic forms, the `Revised` variant and odd-degree pseudo-alternating forms, and the echelonized bases of radicals and complements. The sections on isometry groups, classical groups and Lie algebras wait for Parts IX and XIII. |
